...
void detectKeyPoints(PointCloudPtr &key_points_in, pcl::PointCloud<pcl::PointWithScale>::Ptr &key_points_out)
{
	...
	sift_detect.setScales(0.0005, 4, 5);	//mins scale, numb octaves, num scales per octave
	sift_detect.setMinimumContrast(1.0); // threshold to limit detection
...
void visKeyPoints (const PointCloudPtr points, const pcl::PointCloud<pcl::PointWithScale>::Ptr keypoints, pcl::visualization::PCLVisualizer viz, string cloud, string stringstream)
{
  // Add the points to the vizualizer
  //pcl::visualization::PCLVisualizer viz;
  viz.addPointCloud (points, cloud);
  ...
  // Give control over to the visualizer
 // viz.spin ();
  cout << "completed key point visualization" << endl;
...
void initialRotate(PointCloudPtr point, PointCloudPtr out_point)
{
	float angle = M_PI;
	Eigen::Affine3f transform = Eigen::Affine3f::Identity();
	transform.translation() << 0.0, 0.0, 0.0; // translate with a zoom in Z-axis
	transform.rotate (Eigen::AngleAxisf (angle, Eigen::Vector3f::UnitX())); // x rotation
	pcl::transformPointCloud(*point, *out_point, transform); // transform the cloud
}
				...
				pcl::visualization::PCLVisualizer viz)
{
	...
	const Eigen::Vector3f translate2 (0.1, 0.0, 0.0);
	...
	pcl::transformPointCloud (*points2, *points_right, translate2, no_rotation);
	pcl::transformPointCloud (*keypoints2, *keypoints_right, translate2, no_rotation);
	...
	// Compute the weakest correspondence score
	std::vector<float> temp (correspondence_scores);
	std::sort (temp.begin (), temp.end ());
	float median_score = temp[temp.size ()/2]; // threshold value
	cout << "keypoints_left size: " << keypoints_left->size() << endl;
	
	// Draw lines between the best corresponding points
	for (size_t i = 0; i < keypoints_left->size (); ++i)
	{
		//cout << "corresp score: " << correspondence_scores[i] << endl;
		...
		//cout << endl;
		//cout << "corresp score in:    " << correspondence_scores[i] << endl; 
		//cout << endl;
		...
		//cout << "the keypoints_left: " << keypoints_left->points[i] << endl;
		//cout << "keypoints_right: " << keypoints_right->points[correspondences[i]] << endl;
		...
		//cout << "line added: " << ss.str() << endl;
	}
	// Give control over to the visualizer
	viz.spin ();
	viz.removePointCloud("points_left");
	viz.removePointCloud("points_right");
	viz.removeAllShapes();
}
...
{ // START of MAIN
	...
	//pcl::visualization::PCLVisualizer viewer2("PCView");
...
// ---------------------------------------------------------------------------------------------------------------------
		initialRotate(aPC, aPC);
		initialRotate(bPC,bPC);

		downSample(aPC, aPCSampled);
		downSample(bPC, bPCSampled);
		// display down sampled clouds
		//also need to rotate the initial point clouds to stick with correspondence
		
		//initialRotate(aPCSampled, aPCSampled);
		//initialRotate(bPCSampled, bPCSampled);
	...
	//	viewer.removePointCloud("pcb");
		...
		// detect features 
		detectKeyPoints(aPCSampled, aPC_key);
		detectKeyPoints(bPCSampled, bPC_key);
		// view keypoints only
		//visKeyPoints(aPCSampled, aPC_key, viewer, "apc_point_cloud", "apc_key_cloud_points");
		//visKeyPoints(bPCSampled, bPC_key, viewer2, "bpc_cloudpoint", "bpc_cloud_pointskey_");

		// PFH features
		const float feature_radius = 0.01;
		...
		//  remove the clouds before showing the next clouds
		viewer.removePointCloud("pca");
		viewer.removePointCloud("pcb"); //
		...
		cout << "Corresp = " << corresp[i] << endl;
		cout << "Corresp Score = " << corresp_score[i] << endl;
		...
		cout << "Displaying the correspondences..." << endl;
		visCorresp(aPCSampled, aPC_key, bPCSampled, bPC_key, corresp, corresp_score, viewer);
		...
		cout << "aPC now becomes bPC and we load a new aPC" << endl;
	}
